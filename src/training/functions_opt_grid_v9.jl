...
function predict_with_tuned_neural(Xtr::AbstractMatrix, ytr::Vector{Int}, Xte::AbstractMatrix;
    ...
    best_model  = fitted_params(mach).best_model
    best_report = report(mach)
    # MLJTuning stores the best model's sub-report as best_report.best_report
    # (populated during the train_best=true refit inside MLJ.fit!).
    training_losses = if hasproperty(best_report, :best_report) &&
                         !isnothing(best_report.best_report) &&
                         hasproperty(best_report.best_report, :training_losses)
        best_report.best_report.training_losses
    else
        Float64[]
    end
