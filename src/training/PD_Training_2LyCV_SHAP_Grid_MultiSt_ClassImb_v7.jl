...
# ╔═╡ 670b203c-23f4-4f81-88df-2bbc767ae1b7
begin
    using PlutoUI
...
begin
 # Data handling
	...
	#Class Imbalance
		#needed for ADASYN
	using NearestNeighbors ,LinearAlgebra , Dates

	
    # Import Imbalance.jl for SMOTE-ENN
    using Imbalance
...
html"""
<style>
.plutoui-toc { right: 150px !important; }
</style>
"""
  ╠═╡ =#

# ╔═╡ 08ac3585-adc0-4993-8766-645cce97b06f
md"""
...
begin
	# Import "Parkinsons_Speech-Features.csv" into a data frame
	df = CSV.read("../../data/input/Parkinsons_Speech-Features.csv", DataFrame);
    
    # Extract features and target
    yb = Vector{Int}(df.class) # Target variable
	feature_cols_b = [col for col in names(df) if col ∉ ["id", "gender", "class"]]
    Xb = Matrix{Float64}(df[:, feature_cols_b]) # Features (excl. id, gender and class)
	
	# Feature subsets from paper for comparison with mRMR, Boruta and Person Corr selected features
	X_baseline = 	Xb[:, 1:21]                  
	X_Int =			Xb[:, 22:24]
	X_formfreq =	Xb[:, 25:28]
	X_Bandw =		Xb[:, 29:32]
	X_VocalF =		Xb[:, 33:54]
	X_MFCC = 		Xb[:, 55:138]
	X_wavelet = 	Xb[:, 139:320]                
	X_TQWT = 		Xb[:, 321:752]                   
	
	# Get all feature names for later reference
	feature_names_b = String.(feature_cols_b)    
	
	# Get subject ids (3 records per subject) for later reference
	subjects_b = Vector{Int}(df.id)            

    # Display a summary of dataset dimensions and class dist
	println("Loaded $(size(Xb, 1)) samples with $(size(Xb, 2)) features")
    println("Class distribution: $(countmap(yb))")
end
...
# ===========================================
begin
	feature_cols_temp = [col for col in names(df) if col ∉ ["id", "class"]]

	# Reorder: move sex to the end
    feature_cols_s = [col for col in feature_cols_temp if col != "gender"]
    push!(feature_cols_s, "gender")  # Add sex at the end
	
	Xs = Matrix{Float64}(df[:, feature_cols_s]) # Features (excl. id and class)

# Feature subsets from paper for comparison with mRMR, Boruta and Person Corr selected features
	Xs_baseline =	Xs[:, 1:21]
	Xs_Int =		Xs[:, 22:24]
	Xs_formfreq =	Xs[:, 25:28]
	Xs_Bandw =		Xs[:, 29:32]
	Xs_VocalF =		Xs[:, 33:55]
	Xs_MFCC = 		Xs[:, 55:138]                    
	Xs_wavelet =	Xs[:, 139:320]                
	Xs_TQWT =		Xs[:, 321:752] 
	Xs_Sex = 		Xs[:, 753] 

# Get all feature names for later reference
	feature_names_s = String.(feature_cols_s) 

# Display a summary of dataset dimensions and class dist
	println("Loaded $(size(Xs, 1)) samples with $(size(Xs, 2)) features")
    println("Class distribution: $(countmap(yb))")
...
# ===========================================
#Sex-Specific Aware model
# ===========================================
begin

# Get sex column (last column in Xs)
    sex = Xs[:, 753]
    
    # Create masks for male (1) and female (0)
    male_mask = sex .== 1
    female_mask = sex .== 0
    
    # Filter data by gender (exclude gender column itself)
    Xm = Xs[male_mask, 1:752]      # Male subjects, features 1-752 (no gender)
    Xf = Xs[female_mask, 1:752]    # Female subjects, features 1-752 (no gender)
    
    # Filter labels and subjects by gender
    ym = yb[male_mask]
    yf = yb[female_mask]
    subjects_m = subjects_b[male_mask]
    subjects_f = subjects_b[female_mask]
    
    # Feature names (same as baseline, without gender)
    feature_names_m = feature_names_b  # Male uses baseline feature names
    feature_names_f = feature_names_b  # Female uses baseline feature names

	println("MALE SUBJECTS")
	println("="^60)
	println("Loaded $(size(Xm, 1)) samples with $(size(Xm, 2)) features")
	println("Class distribution: $(countmap(ym))")
    println("Unique subjects: $(length(unique(subjects_m)))")

	 println("\n")
	println("FEMALE SUBJECTS")
	println("="^60)
	println("Loaded $(size(Xf, 1)) samples with $(size(Xf, 2)) features")
	println("Class distribution: $(countmap(yf))")
    println("Unique subjects: $(length(unique(subjects_f)))")
	
end;

# ╔═╡ 88631593-ccbc-4c32-bc17-61f7ca63768f
begin
    # Get unique subjects and their sex
    subject_sex_map = Dict{Int, Float64}()
    
    for (idx, subj_id) in enumerate(subjects_b)
        if !haskey(subject_sex_map, subj_id)
            # Sex is at index 753 in Xs (last column)
            subject_sex_map[subj_id] = Xs[idx, 753]
        end
    end
    
    # Create aligned sex vector (same order as subjects_b)
    sex_vector_b = [subject_sex_map[subj_id] for subj_id in subjects_b]
    
    println("\n" * "="^60)
    println("Subject-level sex vector created")
    println("="^60)
    println("  Total subjects: $(length(unique(subjects_b)))")
    println("  Male subjects: $(sum(sex_vector_b .== 1.0))")
    println("  Female subjects: $(sum(sex_vector_b .== 0.0))")
    println("="^60 * "\n")
end
...
begin
	NNModel = MLJ.@load NeuralNetworkClassifier pkg=MLJFlux verbosity=0
    
# nn_builder with learning_rate parameter:
nn_builder = (n_features, lr=0.0005) -> begin  #reduced from 0.001
...
AdaBoostModel = @load AdaBoostStumpClassifier pkg=DecisionTree verbosity=0
...
md"""
The 2-layer (nested) cross-validation framework implemented in the following code block prevents data leakage by separating feature selection from model evaluation. In the outer loop (5 folds), data is split into training and test sets at the subject level to ensure completely independent evaluation. Within each outer training set, the inner loop (3 folds) independently performs feature selection using three methods (mRMR, Boruta, Pearson Correlation), ensuring that selected features are based only on training data and never exposed to test subjects. Features appearing in at least 2 of the 3 inner folds (consensus voting) are retained for each method. The workflow then evaluates all 6 classifiers on the top features for each method, comparing their performance on truly held-out test subjects. This design ensures unbiased performance estimates by maintaining strict separation between feature selection and final model evaluation.
"""

# ╔═╡ 3a59ce3a-f738-49f7-9f7b-0917f375aadd
md"""
!!! info "ℹ️ CONFIGURATION LAYER: Performance Note"
    **Quick start mode enabled:** Default parameters are set to low values for fast initial runs.
...
begin
    ...
    datasets = Dict( "baseline" => (Xb, feature_names_b, yb, subjects_b),
                     "sex aware" => (Xs, feature_names_s, yb, subjects_b),
                     "male" => (Xm, feature_names_m, ym, subjects_m),
                     "female" => (Xf, feature_names_f, yf, subjects_f)
                   )

    # Initialize results storage for both datasets
    ...
    #Tuning parameters	
	grid_res_multi = g_reso #used SBM-RBF, NN, RF - recommended 7 (7x7 grid)
	grid_res_sing = s_reso #used for Adaboost - recommended 30 
	
	# Reproducibility
    random_seed = 42               # Master seed for reproducible fold assignments
        
    # Performance
    use_multithreading = true      # Parallelize outer folds
    
    # Output control
    verbose_output = true          # Set to true to display verbose progress update
end;

# ╔═╡ 1064fe4e-8c6e-4b74-b979-43a08f34b361
md"""
#### Run Nested CV with ADASYN Strategy

ADASYN (Adaptive Synthetic Sampling) adaptively generates synthetic minority samples
according to their distribution, creating more synthetic data for harder-to-learn cases.
"""

# ╔═╡ 5a24c5a7-9e56-4a0b-a57b-628e45adc207
begin
    # ========================================================================
    # DATASET LOOP: Process each dataset
    # ========================================================================
println("\n" * "="^60)
println("EXECUTING: ADASYN IMBALANCE STRATEGY (Adaptive Synthetic Sampling)")
println("="^60 * "\n")

	# Initialize results storage for ADASYN strategy
    all_results_adasyn = Dict()

	# STRATEGY  Adaptive synthetic sampling
	
   # ========================================================================
    # DATASET LOOP: Process each dataset
    # ========================================================================
    
    for (dataset_name, (X_current, feature_names_current, y_current, subjects_current)) in datasets
        
        println("\n" * "="^60)
        println("Processing dataset: $dataset_name")
        println("="^60 * "\n")
        
        # Assign current dataset to working variables
        X = X_current
        feature_names = feature_names_current
        y = y_current
        subjects = subjects_current
        
        # Initialize results storage
        nested_cv_results = Dict()
        
        # Get outer folds once (deterministic based on random_seed)
        outer_folds = stratified_subject_level_cv(subjects, y, n_outer_folds; 
                                                   random_seed=random_seed)
        
        # Pre-allocate results array for thread-safe storage
        fold_results = Vector{Dict}(undef, n_outer_folds)
        
        # ====================================================================
        # OUTER FOLD LOOP: Parallelized for performance
        # ====================================================================
		
		# Determine if sex splitting should be applied (baseline only)
        sex_vec_to_pass = (dataset_name == "baseline") ? sex_vector_b : nothing

grid_res_multi,	grid_res_sing
		
        if use_multithreading
            Threads.@threads for outer_fold_idx in 1:n_outer_folds
                fold_results[outer_fold_idx] = process_outer_fold(
                    outer_fold_idx, outer_folds[outer_fold_idx],
                    X, y, subjects, feature_names,
                    n_inner_folds, n_features_max, top_n_feats_candidates,
                    consensus_threshold, random_seed,
					grid_res_multi,	grid_res_sing,
					verbose_output;
					sex_vector=sex_vec_to_pass
                )
            end
        else
        ...
        for (idx, result) in enumerate(fold_results)
            nested_cv_results[idx] = result
        end
        
        # Store results for this dataset
        all_results_adasyn[dataset_name] = nested_cv_results
        
    end  # End of dataset loop
   
	# ========================================================================
    # BASELINE SEX VIRTUAL DATASETS
    # ========================================================================
    # Create baseline_male and baseline_female
    # by extracting sex-stratified results from baseline
    
	if haskey(all_results_adasyn, "baseline")
        println("\n" * "="^60)
        println("Materializing baseline sex virtual datasets...")
        println("="^60)
        
        all_results_adasyn["baseline_male"] = materialize_baseline_sex_results(
            all_results_adasyn["baseline"], :male
        )
        all_results_adasyn["baseline_female"] = materialize_baseline_sex_results(
            all_results_adasyn["baseline"], :female
        )
        
        println("✓ Created baseline_male virtual dataset")
        println("✓ Created baseline_female virtual dataset")
    end

	
    println("\n" * "="^60)
    println("Completed ADASYN strategy for all datasets")
    println("="^60)
    println("\nCompleted 2-layer CV")

end

# ╔═╡ 92c1eb07-b9cc-4a44-aede-6b9b1c40fb8e
begin
	# Map dataset names to their data
	...
	# For backward compatibility, keep dataset_map pointing to report version
...
# ========================================================================
# Compute SHAP values for ALL datasets / feature counts
# ========================================================================
begin
 
    # Initialize SHAP storage
    all_datasets_shap_adasyn = Dict()
  
    # Configuration
    local sample_size = samp_sz  # Nr of Monte Carlo samples for SHAP approximation
                
    # Loop through each dataset (CORE datasets only - no virtual datasets)
    for (dataset_name, (X, y, feature_names)) in dataset_map
        
        println("\n" * "="^70)
        println("Computing SHAP values for: $(uppercase(dataset_name))")
        println("="^70)
        
        nested_cv_results = all_results_adasyn[dataset_name]

		# Recompute OUTER FOLDS locally (Pluto-safe, thread-safe)
        ...
        # Store SHAP for each method
        dataset_shap = Dict()
        
        for method in methods
            println("  Processing method: $method")
            
            # Store SHAP for each feature count
            method_shap = Dict()
            
            # Loop through all feature counts
			for n_features in top_n_feats_candidates
                println("    Computing SHAP for $n_features features...")

                # Thread-safe storage
                fold_shap_results =
                    Vector{Dict{String,DataFrame}}(undef, n_outer_folds)

                Threads.@threads for fold_idx in 1:n_outer_folds

            # Fold-specific selected features
  selected_features =nested_cv_results[fold_idx][method]["selected_features_$(n_features)"]

                fold_shap_results[fold_idx] = compute_fold_shap(
                  fold_idx,
                   X,
                   y,
                   selected_features,
                   ...
                   sample_size,
                   random_seed;
                   sex_vector = (dataset_name == "baseline") ? sex_vector_b : nothing
                    )
                end 
			...
			shap_labels =
                    unique(vcat([collect(keys(d)) for d in fold_shap_results]...))

			for shap_label in shap_labels
                    all_shap_values = vcat([d[shap_label] for d in fold_shap_results if haskey(d, shap_label)]...
                    )

    		# Aggregate SHAP values across all folds
    		shap_summary = combine(
                        groupby(all_shap_values, :feature_name),
                        :shap_effect => (x -> mean(abs.(x))) => :mean_abs_shap,
                        :shap_effect => (x -> std(abs.(x)))  => :std_abs_shap)

                    sort!(shap_summary, :mean_abs_shap, rev=true)

    		# Categorise features
shap_summary.feature_name = ["$(fn) ($(categorize_feature_by_index(findfirst(==(fn), feature_names))))" for fn in shap_summary.feature_name ]


    		# Store using (n_features, dataset_label)
    		method_shap[(n_features, shap_label)] = shap_summary
                end
            end
            
            # Store method results
           dataset_shap[method] = method_shap
        end
        
        # Store results for this dataset
         all_datasets_shap_adasyn[dataset_name] = dataset_shap
    end

# ========================================================================
# MATERIALIZE BASELINE SEX VIRTUAL SHAP DATASETS
# ========================================================================

	# Create baseline_male and baseline_female SHAP
    # by extracting sex-stratified SHAP from baseline
    
	if haskey(all_datasets_shap_adasyn, "baseline")
        println("\n" * "="^70)
        println("Materializing baseline sex virtual SHAP datasets...")
        println("="^70)
        
        all_datasets_shap_adasyn["baseline_male"] = materialize_baseline_sex_shap(
            all_datasets_shap_adasyn["baseline"], "baseline_male",
			top_n_feats_candidates
		)
        all_datasets_shap_adasyn["baseline_female"] = materialize_baseline_sex_shap(
            all_datasets_shap_adasyn["baseline"], "baseline_female",
			top_n_feats_candidates
        )
        
        println("✓ Created baseline_male virtual SHAP dataset")
        println("✓ Created baseline_female virtual SHAP dataset")
    end

	
    println("\n" * "="^70)
    println("SHAP computation completed for all datasets and feature counts")
    println("="^70)   
...
begin
    # Create subject-level predictions database
subject_level_db = create_subject_level_database(
    all_results_adasyn,
    subjects_map;
	...
	if files
    	...
    	subject_level_db;
    	filename = "../../outputs/predictions/subject_predictions_detailed.csv",
    	n_outer_folds = n_outer_folds)
...
begin
    ...
    if files
    # Export to CSV
    	CSV.write("../../outputs/predictions/subject_level_data_$(Dates.format(now(), "yyyymmdd_HHMMSS")).csv", subject_level_db)
...
begin
	if files    
    	...
    	export_all_results_to_csv(all_results_adasyn, top_n_feats_candidates,
								  "ADASYN"; output_dir="../../outputs/metrics")
    	...
    	# Save to JLD2
    	results_filename = "../../outputs/models/results_ADASYN_$(Dates.format(now(), "yyyymmdd_HHMMSS")).jld2"
    	...
    	if haskey(all_datasets_shap_adasyn, "baseline")
        	export_baseline_sex_stratified_shap(
				...
				top_n_feats_candidates,
				"ADASYN"; output_dir="../../outputs/shap")
    	end
	else
    	println("Check 'Generate files' to create CSV exports")
...
begin
	...
	if inner_extract_snapshot[] !== nothing
		...
		if _already_done
			...
			# Export CSV
			_csv_fname = "../../outputs/metrics/inner_fold_metrics_ADASYN_$(Dates.format(now(), "yyyymmdd_HHMMSS")).csv"
...
begin
	...
	if @isdefined(inner_fold_df) && nrow(inner_fold_df) > 0
		generate_supplemental_table(inner_fold_df, "ADASYN"; output_dir="../../outputs/metrics")
	else
...
manifest_format = "2.0"
project_hash = "28d010d3e34b8a8ea8950a3c2c6c8cbfcccea5be"
...
deps = ["Dates"]
git-tree-sha1 = "d5e166cb642932a8a3908577dc7fbbe00264b76e"
...
git-tree-sha1 = "e14c278f39ef3bf057d2d1cc95339ef430849017"
...
git-tree-sha1 = "dce1a251ca88c255dd592f4cac960ab8ab28c3b3"
...
git-tree-sha1 = "7b5ef07da5cce32cf33eab3d507f63d73a96de42"
...
git-tree-sha1 = "afcb6562f6db2c14b371e55548196e946aeca967"
