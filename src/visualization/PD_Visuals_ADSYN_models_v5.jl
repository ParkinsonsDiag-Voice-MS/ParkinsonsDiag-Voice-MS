...
# ╔═╡ 0258865f-fc04-4939-9e2a-30d4ccaea741
begin
    using PlutoUI
...
# ============================================
# Import relevant Julia packages
# ============================================
...
begin
 # Data handling
    ...
    using Statistics, StatsBase, Distributions, ROCAnalysis				
    using MultivariateStats: fit
    
    # Machine Learning frameworks
    using MLJ, MLJLIBSVMInterface, MLJDecisionTreeInterface, MLJFlux, 			MLJNaiveBayesInterface, MLJTuning
	# cuDNN, CUDA
    
    # Specific ML algorithms
    using DecisionTree, NaiveBayes, GLM, Flux
    
    # Utilities
    using Random, StableRNGs, PrettyTables, Crayons, Printf, 				Combinatorics
	...
	using InteractiveUtils,Markdown,Base64
...
data = JLD2.load("../../outputs/models/results_ADASYN_20260503_142118.jld2")
...
# Access each variable
	...
	ym = data["ym"]
	yf = data["yf"]
	subjects_b = data["subjects_b"]
	subjects_m = data["subjects_m"]
	subjects_f = data["subjects_f"]
	feature_names_b = data["feature_names_b"]
	feature_names_s = data["feature_names_s"]
	feature_names_m = data["feature_names_m"]
	feature_names_f = data["feature_names_f"]
	sex_vector_b  = data["sex_vector_b"]
	top_n_feats_candidates = data["top_n_feats_candidates"]
	n_outer_folds = data["n_outer_folds"]
    n_inner_folds = data["n_inner_folds"]
    n_features_max = data["n_features_max"]
    consensus_threshold = data["consensus_threshold"]
	random_seed = data["random_seed"]
	all_datasets_shap = data["all_datasets_shap_adasyn"]

	# Core map: used for SHAP computation (original 4 datasets only)
    dataset_map_core = Dict("baseline" => (Xb, yb, feature_names_b),
						   "sex aware" => (Xs, yb, feature_names_s),
						   "male" => (Xm, ym, feature_names_m),
						   "female" => (Xf, yf, feature_names_f)				   
						  )
	
	#Report map: includes baseline sex virtual datasets for plotting/export
	dataset_map_report = Dict("baseline" => (Xb, yb, feature_names_b),
						   "baseline_male" => (Xb, yb, feature_names_b),
						   "baseline_female" => (Xb, yb, feature_names_b),
						   "sex aware" => (Xs, yb, feature_names_s),
						   "male" => (Xm, ym, feature_names_m),
						   "female" => (Xf, yf, feature_names_f)				   
						  )
	
	#For backward compatibility, keeps dataset_map pointing to report version
	dataset_map = dataset_map_report
	
	subjects_map = Dict( "baseline"   => subjects_b,
						 "baseline_male"   => subjects_b,  # Uses baseline subjects
						 "baseline_female" => subjects_b,  # Uses baseline subjects
						 "sex aware"  => subjects_b,
						 "male"       => subjects_m,
						 "female"     => subjects_f,
					   )
	dataset_order = ["baseline",
					 "baseline_male",
					 "baseline_female",
					 "sex aware",
					 "male",
	...
	dataset_order_core = ["baseline", "sex aware", "male", "female"]
	
	methods = ["mRMR", "Boruta"]

  # Map internal dataset keys -> display labels
    dataset_labels = Dict(
    ...
    # Requested line colours
    line_colors = Dict(
        "baseline"  => parse(Colorant, "#648fff"),
        "sex aware" => parse(Colorant, "#dc267f"),
        "male"      => parse(Colorant, "#785ef0"),
        "female"    => parse(Colorant, "#ffb000"),
    )
...
begin
	#if Julia not run from Working directory uncommentbelow
	# cd("/home/rvc/JuliaShare") # write path to the folder
	pwd()
    #both DIR & PWD should Match
	println("DIR = ", @__DIR__)
    println("PWD = ", pwd())

	#call to Function file
	include("../training/functions_opt_grid_v9.jl");
	
	# if above doesnt work uncomment below for Windows or MAC
	#include(joinpath(@__DIR__, "functions_v3.jl"))
	# if above doesnt work update path and uncomment below for Linux
	#include("/home/rvc/JuliaShare/functions_v3.jl")
end

# ╔═╡ 729d199b-50b6-413e-9ce6-d5d3213b44f3
#Call to visualization functions
	# include("visual functions_v3.jl");
	include("visual functions_v4.jl");

# ╔═╡ d92439b1-29e8-45d0-b983-73975620ee13
md"""
# Parkinson diagnosis using voice signal analysis
"""

# ╔═╡ e94b053e-395c-401c-a547-3003526fb7b1
md"""
...
begin
    _inner_fold_csv = "../../outputs/metrics/inner_fold_metrics_ADASYN_20260503_145747.csv"
...
LocalResource("../../outputs/figures/Figure1_Workflow_diagram_simple.jpg")

# ╔═╡ 9280ad56-07aa-4ba0-bd10-f15ad4393c3f
md"""
### Figure 2A: Best Performing Models
"""

# ╔═╡ 693a6685-94ca-4411-8428-f69cab82ea9e
begin
fig2a, bestMCC = quick_mcc_comparison_from_nested_results(all_results,
														  top_n_feats_candidates)
fig2a # fyi, annotated text will sit differently when saved
	
end

# ╔═╡ 3e2b42e6-ed38-49a6-862e-6bfb185c6820
md"""
Note: annotated text will display differently when graph is saved to a file, looks have been adjusted for better quality of saved file.
"""
...
p_fig2a = Plots.plot(fig2a, size=(1600, 1000), dpi=300)
	
savefig(p_fig2a, "../../outputs/figures/Figure2a_bar_chart_best_perf_models.png") # just for local
savefig(p_fig2a, "../../outputs/figures/Figure2a_bar_chart_best_perf_models.pdf") # for ms submission
...
md"""
### Figure 2B: Jaccard Indexes
"""

# ╔═╡ 8dddd871-4e30-4f55-a401-fb2fe0b20d95
begin

	# Extract pre-computed Jaccard indices from all folds and datasets
	jaccard_all_datasets = Dict()

	jaccard_all_datasets = extract_jaccard_indices(all_results,
												   dataset_order_core, 
												   top_n_feats_candidates,
												   verbose=false)

	
	fig2b = plot_jaccard_comparison(jaccard_all_datasets,
								top_n_feats_candidates,
								dataset_labels,
								line_colors;
								method_pair = "mRMR_Boruta",
								# title = "mRMR vs Pearson Agreement",
								size = (950, 600),
								dpi = 300)
...
Image file generation
"""
	p_fig2b = Plots.plot(fig2b, size=(1600, 1000), dpi=300)
	savefig(p_fig2b, "../../outputs/figures/Figure2b_jaccard_index_by_nfeatures.png") # just for local
	savefig(p_fig2b, "../../outputs/figures/Figure2b_jaccard_index_by_nfeatures.pdf") # for ms submission
end;

# ╔═╡ eaae31bd-ad32-4c9b-8305-e5525156fec4
md"""
### Figure 3: Top 3 Classifier-Method Combinations by Dataset
"""
...
classifiers = [
	...
	classifier_display_names = Dict(
        "RandomForest"        => "Random Forests",
        "RandomForest_TUNED"  => "Random Forests", #"Random Forest (Tuned)"
        "NeuralNetwork"       => "Neural Network",
        "NeuralNetwork_TUNED" => "Neural Network", #"Neural Network (Tuned)"
        "NaiveBayes"          => "Naive Bayes",
        "LogisticRegression"  => "Logistic Regression",
        "AdaBoost"            => "AdaBoost",
        "AdaBoost_TUNED"      => "AdaBoost", #AdaBoost (Tuned)"
        "SVM_RBF"             => "SVM-RBF",
        "SVM_RBF_TUNED"       => "SVM-RBF") #"SVM-RBF (Tuned)")

    classifier_colors = Dict(
        "RandomForest"        => "#dc267f",
        "RandomForest_TUNED"  => "#dc267f",
        "NeuralNetwork"       => "#785ef0",
        "NeuralNetwork_TUNED" => "#785ef0",
        "NaiveBayes"          => :mediumseagreen,
        "LogisticRegression"  => :purple,
        "AdaBoost"            => "#ffb000",
        "AdaBoost_TUNED"      => "#ffb000",
        "SVM_RBF"             => "#fe6100",
        "SVM_RBF_TUNED"       => "#fe6100")

	classifier_markers = Dict(
        "RandomForest"        => :circle,
        "RandomForest_TUNED"  => :circle,
        "NeuralNetwork"       => :square,
        "NeuralNetwork_TUNED" => :square,
        "NaiveBayes"          => :diamond,
        "LogisticRegression"  => :utriangle,
        "AdaBoost"            => :star5,
        "AdaBoost_TUNED"      => :star5,
        "SVM_RBF"             => :hexagon,
...
p_fig3 = Plots.plot(fig3..., layout=(2, 2),
			  size=(1600, 1000),
			  dpi=300)
	
savefig(p_fig3, "../../outputs/figures/Figure3_Top_3_Classifier-Method_Combo.png") # just for local
savefig(p_fig3, "../../outputs/figures/Figure3_Top_3_Classifier-Method_Combo.pdf") # for ms submission
...
begin
cm_csv_path = "../../outputs/tables/Best_models_confusion_matrices_table.csv"
    df = CSV.read(cm_csv_path, DataFrame)

    # Column names (your updated CSV)
    col_model = "Model configuration"
    col_fs    = "FS method"
    col_n     = "N features"
    col_clf   = "Classifier"
    col_act   = "CF Actual"
    col_pHC   = "Predicted HC"
    col_pPD   = "Predicted PD"

    # ---- robust parsing helpers ----
    to_str(x) = ismissing(x) ? "" : strip(string(x))

# Find header row for each model block (row where CF Actual == "Actual")
    starts = Int[]
    for i in 1:nrow(df)
        m = df[i, col_model]
        a = df[i, col_act]
        if !ismissing(m) && !ismissing(a) && to_str(a) == "Actual"
            push!(starts, i)
        end
    end

    # Parse each model block into a Dict
    models = Dict{String, NamedTuple}()

    for i in starts
        if i + 2 > nrow(df)
            @warn "Skipping incomplete block at end of CSV" i
            continue
        end

        model = to_str(df[i, col_model])
        fs    = to_str(df[i, col_fs])
        nfeat = to_int(df[i, col_n])
        clf   = to_str(df[i, col_clf])

        hc_hc = to_int(df[i+1, col_pHC])
        hc_pd = to_int(df[i+1, col_pPD])
        pd_hc = to_int(df[i+2, col_pHC])
        pd_pd = to_int(df[i+2, col_pPD])

        if any(ismissing, (nfeat, hc_hc, hc_pd, pd_hc, pd_pd))
            @warn "Skipping model block due to missing/unparseable values" model i
            continue
        end

        # rows = Actual [HC, PD], cols = Predicted [HC, PD]
        cm = [hc_hc hc_pd;
              pd_hc pd_pd]

        models[model] = (fs=fs, nfeat=nfeat, clf=clf, cm=cm)
    end

    # Desired order (2×3)
    order = [
        "Baseline (pooled; sex excluded)",
        "Baseline model, male outputs",
        "Baseline model, female outputs",
        "Sex-aware (pooled; sex included)",
        "Male-only model",
        "Female-only model",
    ]
    ...
    # Fixed colors for quadrants (no color scale)
    col_TN = parse(Colorant, "#648fff")  # Actual HC / Pred HC
    col_FP = parse(Colorant, "#dc267f")  # Actual HC / Pred PD
    col_FN = parse(Colorant, "#785ef0")  # Actual PD / Pred HC
    col_TP = parse(Colorant, "#ffb000")  # Actual PD / Pred PD

	
    # Plot one panel (counts + row-normalized %), with fixed quadrant colors
    function plot_cm_panel!(ax, cm_counts::Matrix{Int}, title::String)
        rs = sum(cm_counts, dims=2)
        rn = Float64.(cm_counts) ./ rs
        rn[.!isfinite.(rn)] .= 0.0
        ...
        for i in 1:2, j in 1:2
            CairoMakie.poly!(ax, Rect(j-0.5, i-0.5, 1, 1);
                color = colors[i,j],
                strokecolor = :white,
                strokewidth = 2
            )
        ...
        # Text annotations (counts + row %), always white
        for i in 1:2, j in 1:2
            c = cm_counts[i, j]
            p = rn[i, j] * 100
...
Image file generation
"""
    CairoMakie.save("../../outputs/figures/Figure4_Confusion_Matrices.png",
					fig4; px_per_unit=2,
					backgroundcolor=:white) 			# for local
    CairoMakie.save("../../outputs/figures/Figure4_Confusion_Matrices.pdf",
					fig4;
					backgroundcolor=:white)				# for ms submission
end;

# ╔═╡ 41f8dcef-516d-47ef-93b9-24ddd4edb56a
md"""
###  Figure 5: SHAP-based Feature Importance 
"""
...
begin
	...
	male_icon_b64 = base64encode(read("../../assets/male_symbol.png"))
    female_icon_b64 = base64encode(read("../../assets/female_symbol.png"))
	
	# Load SHAP data
    male_shap = CSV.read("../../outputs/shap/male_shap.csv", DataFrame)
    female_shap = CSV.read("../../outputs/shap/female_shap.csv", DataFrame)
    
    # Extract category from feature name
    function extract_category(s)
        ismissing(s) && return "Unknown"
        m = match(r"\(([^)]+)\)$", s)
        return m !== nothing ? m.captures[1] : "Unknown"
    end
    
    # Heatmap table function
    function shap_heatmap_table(male_shap_df::DataFrame, female_shap_df::DataFrame; 
                                n_features::Int=10,
                                title::String="SHAP Feature Importance: Male vs Female")
        
        # Get top N from each group
        male_top = first(sort(male_shap_df, :MeanAbsSHAP, rev=true), n_features)
        female_top = first(sort(female_shap_df, :MeanAbsSHAP, rev=true), n_features)
        male_top_features = Set(male_top.Feature)
        female_top_features = Set(female_top.Feature)
        
        # Find intersection and exclusive sets
        both_features = intersect(male_top_features, female_top_features)
        male_only_features = setdiff(male_top_features, female_top_features)
        female_only_features = setdiff(female_top_features, male_top_features)
        
        # Create lookup dictionaries
        male_lookup = Dict(row.Feature => row.MeanAbsSHAP for row in eachrow(male_shap_df))
        female_lookup = Dict(row.Feature => row.MeanAbsSHAP for row in eachrow(female_shap_df))
        
        # Build combined dataframe with group labels
        rows = []
        
        # Group 1: Both (sorted by max SHAP)
        for feat in both_features
            male_val = get(male_lookup, feat, 0.0)
            female_val = get(female_lookup, feat, 0.0)
            push!(rows, (Feature=feat, Male=male_val, Female=female_val, 
                        MaxSHAP=max(male_val, female_val), Group="Both",
                        Category=extract_category(feat)))
        end
        
        # Group 2: Male only (sorted by male SHAP)
        for feat in male_only_features
            male_val = get(male_lookup, feat, 0.0)
            female_val = get(female_lookup, feat, 0.0)
            push!(rows, (Feature=feat, Male=male_val, Female=female_val,
                        MaxSHAP=male_val, Group="Male",
                        Category=extract_category(feat)))
        end
        
        # Group 3: Female only (sorted by female SHAP)
        for feat in female_only_features
            male_val = get(male_lookup, feat, 0.0)
            female_val = get(female_lookup, feat, 0.0)
            push!(rows, (Feature=feat, Male=male_val, Female=female_val,
                        MaxSHAP=female_val, Group="Female",
                        Category=extract_category(feat)))
        end
        
        combined_df = DataFrame(rows)
        
        # Sort within groups: Both first, then Male-only, then Female-only
        # Within each group, sort by MaxSHAP descending
        group_order = Dict("Both" => 1, "Male" => 2, "Female" => 3)
        sort!(combined_df, [:Group, :MaxSHAP], by=[x -> group_order[x], identity], rev=[false, true])
        
        global_max = max(maximum(combined_df.Male), maximum(combined_df.Female))
        
		# Function to define heatmap colors
		function value_to_color(val, max_val)
		    if val == 0 || ismissing(val)
		        return "background-color: #f8f9fa;"
		    end
		    intensity = val / max_val
		    r = round(Int, 238 - (238 - 100) * intensity)  # 238 -> 100
		    g = round(Int, 242 - (242 - 70) * intensity)   # 242 -> 70
		    b = round(Int, 255 - (255 - 225) * intensity)  # 255 -> 225
		    return "background-color: rgb($r, $g, $b);"
		end
        
        # Function to define category colors
		function category_color(cat)
            colors = Dict(
                ...
                "Bandwidth" => "#06b6d4",
                "Intensity" => "#ec4899",
                "Formant Frequency" => "#6366f1"
            )
            return get(colors, cat, "#6b7280")
        end
		...
		for row in eachrow(combined_df)
            feature_name = replace(row.Feature, r"\s*\([^)]+\)$" => "")
            male_val = row.Male
            female_val = row.Female
            category = row.Category
            group = row.Group
            
            male_style = value_to_color(male_val, global_max)
            female_style = value_to_color(female_val, global_max)
            cat_color = category_color(category)
            
            male_weight = male_val >= female_val ? "font-weight: bold;" : ""
            female_weight = female_val > male_val ? "font-weight: bold;" : ""
...
<td style="text-align: center; padding: 8px 12px; border-bottom: 1px solid #e5e7eb; width: 180px; font-size: 18px; $female_style $female_weight">$(@sprintf("%.3f", female_val))</td>                <td style="text-align: center; padding: 8px 12px; border-bottom: 1px solid #e5e7eb;">
        ...
        n_both = count(combined_df.Group .== "Both")
        n_male = count(combined_df.Group .== "Male")
        n_female = count(combined_df.Group .== "Female")
...
<th style="text-align: center; padding: 10px 12px; border-bottom: 2px solid #d1d5db; font-weight: 600; font-size: 18px; color: #d02670; width: 180px;">Female</th>                        <th style="text-align: center; padding: 10px 12px; border-bottom: 2px solid #d1d5db; font-weight: 600;">Cat.</th>
            ...
            <div style="display: flex; gap: 20px; flex-wrap: wrap; justify-content: center; font-size: 18px; margin-bottom: 15px;">
                ...
                <div style="display: flex; align-items: center; gap: 5px;">
                    <img src="data:image/png;base64,$male_icon_b64" style="height: 22px; vertical-align: middle;" />
                    <span>Male only</span>
                </div>
                <div style="display: flex; align-items: center; gap: 5px;">
                    <img src="data:image/png;base64,$female_icon_b64" style="height: 22px; vertical-align: middle;" />
            ...
            <div style="display: flex; gap: 20px; flex-wrap: wrap; justify-content: center; font-size: 18px;">
                ...
                <div style="display: flex; align-items: center; gap: 5px;">
                    <span style="display: inline-block; width: 12px; height: 12px; border-radius: 50%; background-color: #785EF0;"></span>
                    <span>TQWT</span>
                </div>
                <div style="display: flex; align-items: center; gap: 5px;">
                    <span style="display: inline-block; width: 12px; height: 12px; border-radius: 50%; background-color: #DC267F;"></span>
                    <span>Vocal Fold</span>
                </div>
                <div style="display: flex; align-items: center; gap: 5px;">
                    <span style="display: inline-block; width: 12px; height: 12px; border-radius: 50%; background-color: #FFB000;"></span>
                    <span>Baseline</span>
                </div>
    ...
    # Clean data
    male_clean = dropmissing(male_shap, :Feature)
...
begin
	...
	xmax = ceil(max_val / 0.02) * 0.02
	xt = 0.0:0.02:xmax

    # Function to create SHAP horizontal barplot
	function shap_barh(df, title_str; flip=false, bar_color=colorant"#7f8c8d")
        df = sort(df, :Mean_SHAP, rev=true)
	
	    x = Float64.(df.Mean_SHAP)
	    y = String.(df.Feature)
	
	    if flip # bars go left
	        x = .-x                      
	        tick_pos = collect(-xmax:0.02:0.0)
	        tick_lab = string.(abs.(tick_pos))
	        xlims_ = (-xmax, 0.0)
    ...
    # Call function to create male and female bar plots
	p_male   = shap_barh(male_shap_df,   ""; flip=true,  bar_color=male_bar_color)
    p_female = shap_barh(female_shap_df, "";            bar_color=female_bar_color)
...
md"""
###  Table 1: Best-performing model configurations
"""

# ╔═╡ 0dff516f-5670-4e2a-8003-183d27ef9a0d
begin
	################################################################################
	# Table 1: Best-performing model configurations (ADASYN; outer-fold mean over 5)
	################################################################################
	...
	paths = Dict(
	    :baseline        =>
			"../../outputs/metrics/results_baseline_(ADASYN)_20260503_142118.csv",
	    :sex_aware       => "../../outputs/metrics/results_sex aware_(ADASYN)_20260503_142118.csv",
	    :male            => "../../outputs/metrics/results_male_(ADASYN)_20260503_142117.csv",
	    :female          => "../../outputs/metrics/results_female_(ADASYN)_20260503_142118.csv",
	    :baseline_male   => "../../outputs/metrics/results_baseline_male_(ADASYN)_20260503_142122.csv",
	    :baseline_female => "../../outputs/metrics/results_baseline_female_(ADASYN)_20260503_142122.csv",
	)
	
	dfs = Dict(k => CSV.read(v, DataFrame) for (k,v) in paths)
	
	# --- 2) Summarise (mean + SD across outer folds) for each config ---
	# Columns in your CSVs: Dataset, Method, N_Features, Classifier, Fold,
	#                       MCC, F1, Sensitivity, Specificity, Balanced_Accuracy, Accuracy
	function summarise_configs(df::DataFrame)
	    g = groupby(df, [:Dataset, :Method, :N_Features, :Classifier])
	
	    out = combine(g,
	        :Fold => (x -> length(unique(x))) => :n_folds,
	        :MCC  => mean => :MCC_mean,  :MCC  => std => :MCC_sd,
	        :F1   => mean => :F1_mean,   :F1   => std => :F1_sd,
	        :Balanced_Accuracy => mean => :BalAcc_mean, :Balanced_Accuracy => std => :BalAcc_sd,
	        :Sensitivity       => mean => :Sens_mean,   :Sensitivity       => std => :Sens_sd,
	        :Specificity       => mean => :Spec_mean,   :Specificity       => std => :Spec_sd,
	        :Accuracy          => mean => :Acc_mean,    :Accuracy          => std => :Acc_sd,
	    )
	
	    # sanity check: these files should represent 5 outer folds per config
	    if any(out.n_folds .!= 5)
	        @warn "Some configurations do not have 5 unique folds. Inspect `n_folds`." unique(out.n_folds)
	    end
	
	    return out
	end
	
	summ = Dict(k => summarise_configs(v) for (k,v) in dfs)
	
	# --- 3) Pick best config by MCC (descending) for each dataset ---
	best_by_mcc(sdf::DataFrame) = first(sort(sdf, :MCC_mean, rev=true), 1)
	
	best_baseline  = best_by_mcc(summ[:baseline])
	best_sexaware  = best_by_mcc(summ[:sex_aware])
	best_male      = best_by_mcc(summ[:male])
	best_female    = best_by_mcc(summ[:female])
	
	# --- 4) For sex-stratified baseline evaluation:
	#        Use the *baseline-best* configuration and extract its performance
	#        from baseline_male / baseline_female files.
	function get_config_row(sdf::DataFrame; method::AbstractString, nfeat::Int, clf::AbstractString)
	    r = filter(row -> row.Method == method &&
	                      row.N_Features == nfeat &&
	                      row.Classifier == clf, sdf)
	    isempty(r) && error("Config not found in summary table: Method=$method, N_Features=$nfeat, Classifier=$clf")
	    return first(r, 1)
	end
	
	base_method = best_baseline.Method[1]
	base_nfeat  = best_baseline.N_Features[1]
	base_clf    = best_baseline.Classifier[1]
	
	baseline_eval_male   = get_config_row(summ[:baseline_male];   method=base_method, nfeat=base_nfeat, clf=base_clf)
	baseline_eval_female = get_config_row(summ[:baseline_female]; method=base_method, nfeat=base_nfeat, clf=base_clf)
	
	# --- 5) Assemble Table 2 with BMJ-style "mean (SD)" formatting ---
	# fmt(ms, ss; d=3) = @sprintf("%0.*f (%0.*f)", d, ms, d, ss
	fmt(ms, ss; d=3) = @sprintf("%0.*f ± %0.*f", d, ms, d, ss)

	
	function pack_row(label::AbstractString, r::DataFrameRow)
	    return (
	        Model = label,
	        FS_Method = String(r.Method),
	        N_Features = Int(r.N_Features),
	        Classifier = String(r.Classifier),
	        MCC = fmt(r.MCC_mean, r.MCC_sd),
	        F1 = fmt(r.F1_mean, r.F1_sd),
	        BalAcc = fmt(r.BalAcc_mean, r.BalAcc_sd),
	        Sens = fmt(r.Sens_mean, r.Sens_sd),
	        Spec = fmt(r.Spec_mean, r.Spec_sd),
	        # Acc = fmt(r.Acc_mean, r.Acc_sd),
	    )
	end
	
	table2 = DataFrame([
	    pack_row("Baseline (pooled; sex excluded)", best_baseline[1, :]),
		pack_row("Baseline model, male outputs", baseline_eval_male[1, :]),
	    pack_row("Baseline model, female outputs", baseline_eval_female[1, :]),
	    pack_row("Sex-aware (pooled; sex included)", best_sexaware[1, :]),
	    pack_row("Male-only model", best_male[1, :]),
	    pack_row("Female-only model", best_female[1, :]),

	])
	
	# --- 6) Display / export ---
	# Give Table 2 BMJ-style column names
	rename!(table2, Dict(
	...
	################################################################################
	# * Footnote:
	# Sex-stratified performance for the SAME pooled baseline-best configuration
	# (i.e., the configuration selected as best in the pooled baseline table).
	################################################################################

	# Print 
	# pretty_table(table2) -- commented out; doesn't show full cols well.
		
	# Save
		CSV.write("../../outputs/tables/Table1_ADASYN_best_configs.csv", table2)
...
begin
    if inner_fold_df === nothing
        ...
        best_configs_t2 = Dict(
            "baseline"        => (method="Boruta", n_features=100,
								  classifier="NeuralNetwork"),
            "baseline_male"   => (method="Boruta", n_features=100,
								  classifier="NeuralNetwork"),
            "baseline_female" => (method="Boruta", n_features=100,
								  classifier="NeuralNetwork"),
            "sex aware"       => (method="Boruta",   n_features=80,
        ...
        CSV.write("../../outputs/tables/Table2_inner_fold_summary.csv", tbl2)
...
begin
    if inner_fold_df === nothing
        ...
        for (ds, meth, nfeat) in _best_configs_f6
            ...
            tag = replace(ds, " " => "_")
            CairoMakie.save("../../outputs/figures/Figure6_InnerOuter_MCC_$(tag).png", fig6;
                px_per_unit = 2, backgroundcolor = :white)
            CairoMakie.save("../../outputs/figures/Figure6_InnerOuter_MCC_$(tag).pdf", fig6;
...
begin
    if nn_training_losses === nothing
        ...
        for (ds, (meth, fkey)) in _nn_configs
            ...
            tag  = replace(ds, " " => "_")
            CairoMakie.save("../../outputs/figures/Figure7_NN_loss_$(tag).png", fig7;
                px_per_unit = 2, backgroundcolor = :white)
            CairoMakie.save("../../outputs/figures/Figure7_NN_loss_$(tag).pdf", fig7;
                backgroundcolor = :white)
        end
...
begin
    ...
    elseif length(unique(inner_fold_df.N_Features)) <= 1
        ...
        for ds in ["baseline", "sex aware", "male", "female"]
            ...
            tag = replace(ds, " " => "_")
            CairoMakie.save("../../outputs/figures/Figure8_ValMCC_vs_N_$(tag).png", fig8;
                px_per_unit = 2, backgroundcolor = :white)
            CairoMakie.save("../../outputs/figures/Figure8_ValMCC_vs_N_$(tag).pdf", fig8;
                backgroundcolor = :white)
        end
...
git-tree-sha1 = "cb65604a17525c341222e5379aad3dc6d322e719"
...
git-tree-sha1 = "7555ac74e0c16a048675267eae68c5b247a72161"
